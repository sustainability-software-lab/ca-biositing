-- ============================================================================
-- XRD Analysis Comprehensive Audit
-- ============================================================================
...
SELECT
    ...
    ROUND(100.0 * COUNT(DISTINCT o.record_id) / NULLIF((SELECT COUNT(*) FROM xrd_record WHERE qc_pass != 'fail'), 0), 1) AS record_coverage_pct
FROM observation o
JOIN parameter p ON o.parameter_id = p.id
LEFT JOIN unit u ON o.unit_id = u.id
JOIN xrd_record r ON o.record_id = r.record_id
WHERE o.record_type = 'xrd analysis' AND r.qc_pass != 'fail'
GROUP BY p.name
