-- ============================================================================
-- Pretreatment Comprehensive Audit
-- ============================================================================

-- 1. RECORD COUNTS & QC GOVERNANCE
SELECT
    'pretreatment' AS record_type,
...
-- 2. METADATA & VESSEL COMPLETENESS (QC-PASS/NULL ONLY)
SELECT
    'pretreatment' AS record_type,
...
SELECT
    ...
    ROUND(100.0 * COUNT(DISTINCT o.record_id) / NULLIF((SELECT COUNT(*) FROM pretreatment_record WHERE qc_pass != 'fail'), 0), 1) AS record_coverage_pct
FROM observation o
JOIN parameter p ON o.parameter_id = p.id
LEFT JOIN unit u ON o.unit_id = u.id
JOIN pretreatment_record r ON o.record_id = r.record_id
WHERE o.record_type = 'pretreatment' AND r.qc_pass != 'fail'
GROUP BY p.name
