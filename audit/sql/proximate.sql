-- ============================================================================
-- Proximate Analysis Comprehensive Audit
-- ============================================================================
...
SELECT
    'proximate' AS record_type,
    COUNT(DISTINCT pr.prepared_sample_id) AS unique_prepared_samples,
    COUNT(DISTINCT pr.resource_id) AS unique_resources,
    COUNT(DISTINCT pr.experiment_id) AS unique_experiments,
    COUNT(DISTINCT fs.id) AS unique_field_samples,
    COUNT(DISTINCT pap.id) AS unique_primary_ag_products,
    ROUND(100.0 * COUNT(DISTINCT pap.id) / NULLIF(COUNT(DISTINCT pr.prepared_sample_id), 0), 1) AS lineage_integrity_pct
FROM proximate_record pr
LEFT JOIN prepared_sample ps ON pr.prepared_sample_id = ps.id
...
SELECT
    ...
    ROUND(100.0 * COUNT(DISTINCT o.record_id) / NULLIF((SELECT COUNT(*) FROM proximate_record WHERE qc_pass != 'fail'), 0), 1) AS record_coverage_pct
FROM observation o
JOIN parameter p ON o.parameter_id = p.id
LEFT JOIN unit u ON o.unit_id = u.id
JOIN proximate_record r ON o.record_id = r.record_id
WHERE o.record_type = 'proximate analysis' AND r.qc_pass != 'fail'
GROUP BY p.name
ORDER BY obs_count DESC;

-- 5. VALUE RANGES & OUTLIER DETECTION (QC-PASS/NULL ONLY)
SELECT
    p.name AS parameter,
    MIN(o.value) AS min_val,
    MAX(o.value) AS max_val,
    ROUND(AVG(o.value)::numeric, 4) AS avg_val,
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY o.value) AS median_val,
    COUNT(*) FILTER (WHERE o.value < 0) AS negative_values_count
FROM observation o
JOIN parameter p ON o.parameter_id = p.id
JOIN proximate_record r ON o.record_id = r.record_id
WHERE o.record_type = 'proximate analysis' AND r.qc_pass != 'fail' AND o.value IS NOT NULL
GROUP BY p.name;
