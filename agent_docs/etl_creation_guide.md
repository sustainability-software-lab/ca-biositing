...
| `Session(bind=conn)` silently fails in SQLAlchemy 2.x, writing zero rows with no visible error                                                | Use `Session(engine)` + `with session.begin():` — never `Session(bind=conn)`                                                                         |
| Duplicate Google Sheet column names silently dropped by `df.columns.duplicated()`, losing data (e.g. multiple `Quantity (tons/year)` columns) | Use a `_dedupe_columns()` helper that renames duplicates with `_2`, `_3`, … suffixes instead of dropping them                                        |
| Comma-separated string fields contain `","` or `",,"` when source cells are empty                                                             | Filter out empty/`NaN`/`"nan"` values before `", ".join()`; return `None` (not `""`) when the filtered list is empty                                 |
| Secret env vars (e.g. `GOOGLE_MAPS_API_KEY`) set in root `.env` but never reach the Docker container                                          | Docker Compose resolves `env_file` relative to the `docker-compose.yml` location — add secrets to `resources/docker/.env`, not the repo root `.env`  |
| New flow never appears in `pixi run run-etl` because it wasn't registered                                                                     | Add the flow to `AVAILABLE_FLOWS` in `resources/prefect/run_prefect_flow.py`                                                                         |
| Google Sheet has a spurious frozen/merged title row above real headers, causing all column lookups to fail silently                           | Add a `_fix_header_row()` guard in the transform that detects and promotes the real header row when the first row doesn't match expected field names |
