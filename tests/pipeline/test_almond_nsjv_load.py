...
def test_load_county_ag_reports_upserts_and_spot_checks(monkeypatch):
    from ca_biositing.datamodels.models import Observation, Parameter, Resource, ResourcePriceRecord, Unit
    ...
    with Session(engine) as session:
        ...
        price_received_parameter = session.exec(select(Parameter).where(Parameter.name == "price received")).first()
        assert price_received_parameter is not None
        assert price_received_parameter.standard_unit_id == unit.id

        loaded_price_record = session.exec(select(ResourcePriceRecord)).first()
        assert loaded_price_record is not None

        loaded_observation = session.exec(
            select(Observation).where(Observation.parameter_id == price_received_parameter.id)
        ).first()
        assert loaded_observation is not None
        assert loaded_observation.record_id == str(loaded_price_record.id)
